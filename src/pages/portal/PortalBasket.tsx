...
export default function PortalBasket() {
  ...
  const listTotal = items.reduce((s, i) => s + i.list_price_usd * i.quantity, 0);
  const partnerTotal = items.reduce(
    (s, i) => s + i.list_price_usd * (1 - discount) * i.quantity,
    0
  );
    ...
    if (removed) {
      toast("Item removed", {
        ...
        action: {
          label: "Undo",
          onClick: () => {
            // Re-add the item — useBasket.addItem handles it
            const { useBasket: _ , ...rest } = {} as any;
            // We need a ref to addItem, but since toast is sync we lose it
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="hidden md:block bg-card rounded-xl border shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((item) => {
                    const partnerPrice = item.list_price_usd * (1 - discount);
